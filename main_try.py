...
print h_conv2
w_conv3 = weight_variable([1,1,k_filter,1])
b_conv3 = bias_variable([1])
h_conv3=  tf.nn.relu(conv2d(h_conv2, w_conv3) + b_conv3)

h_pool3_flat = tf.reshape(h_conv3, [-1 ,15*15*1])
print h_pool3_flat
...
prediction = tf.nn.softmax(h_pool3_flat)
print prediction
cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(h_pool3_flat, prediction))
...
for  i in range(100):
    ...
    all_layer_1 = game.ReturnAllInfo (set_x, cut_color)
    bbb =  sess.run(optimizer, feed_dict = {input: np.reshape(all_layer_1,[1,15,15,24]),output : np.reshape(out_y,[1,225])})
...
for i in range(100):
    ...
    h = sess.run(cross_entropy, feed_dict={input: np.reshape(all_layer_1,[1,15,15,24]), output: np.reshape(out_y,[1,225])})
    print h
    a = raw_input()
    ...
    h = sess.run(h_conv1, feed_dict={input: np.reshape(all_layer_1,[1,15,15,24]), output: np.reshape(out_y,[1,225])})
    print h[0]
    a = raw_input()
    h = sess.run(h_conv3, feed_dict={input: np.reshape(all_layer_1,[1,15,15,24]), output: np.reshape(out_y,[1,225])})
    print h[0]
    a = raw_input()
    ...
    pre_num = int(pre[0])
    game.show_game(np.reshape(set_x,[225,1]))
    game.show_game(np.reshape(out_y,[225,1]))
    game.show_game_pos(pre_num)
    print pre_num
    game.show_game_set(pre_num)
    a = raw_input()
